				...
				for other_page in before_pages {
					if page_has_rule_for_other_page(page, other_page) &&
					   !page_order_satisfied(page, other_page, .Before) {
						sort_manual_update_pages(&update)
						sum += u32(update.pages[len(update.pages) / 2])
...
sort_manual_update_pages :: proc(manual_update: ^Manual_Update) {
	slice.sort_by(manual_update.pages, proc(i: u8, j: u8) -> bool {
		return !page_has_rule_for_other_page(i, j) || page_order_satisfied(i, j, .Before)
	})
}
